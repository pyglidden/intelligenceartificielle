...
#define SHOAL
  ...
  m_Leader->Steering()->FlockingOn();
  m_Leader->Steering()->WanderOn();
  m_Leader->SetMaxSpeed(60);
  ...
  for (int i=0; i<Prm.NumAgents; ++i)
  {
	  ...
	  */
	  ...
	  if (i > 1) {
		  if (sign < 0) {
			  m_Followers[i]->Steering()->OffsetPursuitOn(m_Followers[i - 2], Vector2D(x, y - 20));	// impair
		  }
		  else if (sign > 0)
			  m_Followers[i]->Steering()->OffsetPursuitOn(m_Followers[i - 2], Vector2D(x, y + 20));	// pair
	  }
	  else {
		  if (sign < 0) {
			  m_Followers[i]->Steering()->OffsetPursuitOn(m_Leader, Vector2D(x, y - 20));		// 1
		  }
		  else if (sign > 0)
			  m_Followers[i]->Steering()->OffsetPursuitOn(m_Leader, Vector2D(x, y + 20));		// 0 
	  }
  }
#endif
...
void GameWorld::HandleMenuItems(WPARAM wParam, HWND hwnd)
{
  switch(wParam)
  {
    ...
    case IDM_PARTITION_VIEW_NEIGHBORS:
	  ...
	  //TODO Leaderwander menu
	case ID_LEADER_WANDER:
	case ID_LEADER_SEEK:
	{
		if (m_Leader->Steering()->isWanderOn()) {
			m_Leader->Steering()->WanderOff();
			m_Leader->Steering()->FlockingOff();
			m_Leader->Steering()->SeekOn();
			ChangeMenuState(hwnd, ID_LEADER_WANDER, MFS_UNCHECKED);
			ChangeMenuState(hwnd, ID_LEADER_SEEK, MFS_CHECKED);
		} else {
			m_Leader->Steering()->FlockingOn();
			m_Leader->Steering()->WanderOn();
			m_Leader->Steering()->SeekOff();
			ChangeMenuState(hwnd, ID_LEADER_SEEK, MFS_UNCHECKED);
			ChangeMenuState(hwnd, ID_LEADER_WANDER, MFS_CHECKED);
		}
	}
	break;
	...
	case ID_FORMATION_L:
	{
		m_Followers[0]->Steering()->CohesionOn();
		m_Followers[0]->Steering()->AlignmentOn();
		m_Followers[0]->Steering()->SeparationOn();
		m_Followers[0]->Steering()->OffsetPursuitOn(m_Leader, Vector2D(-30, 0));
		...
		for (int i = 1; i < Prm.NumAgents; ++i)
		{
			...
			m_Followers[i]->Steering()->AlignmentOn();
			m_Followers[i]->Steering()->SeparationOn();
			m_Followers[i]->Steering()->OffsetPursuitOn(m_Followers[i-1], Vector2D(-30, 0));
		}
		ChangeMenuState(hwnd, ID_FORMATION_V, MFS_UNCHECKED);
		ChangeMenuState(hwnd, ID_FORMATION_L, MFS_CHECKED);
		ChangeMenuState(hwnd, ID_FORMATION_O, MFS_UNCHECKED);
	}
	break;
	...
	{
		int rayon = 60;
		float angle = 360 / Prm.NumAgents;
		float x = 0, y = 0;

		for (int i = 0; i < Prm.NumAgents; ++i)
		{
			float FollowerAngle = angle * i;
			x = rayon * cos(FollowerAngle);
			y = rayon * sin(FollowerAngle);

			m_Followers[i]->Steering()->CohesionOn();
			m_Followers[i]->Steering()->AlignmentOn();
			m_Followers[i]->Steering()->SeparationOn();
			m_Followers[i]->Steering()->OffsetPursuitOn(m_Leader, Vector2D(x, y));
		}
		ChangeMenuState(hwnd, ID_FORMATION_O, MFS_CHECKED);
		ChangeMenuState(hwnd, ID_FORMATION_L, MFS_UNCHECKED);
		ChangeMenuState(hwnd, ID_FORMATION_V, MFS_UNCHECKED);
	} 
	break; 
	
	case ID_FORMATION_V: 
	{	//sinon on change pour v
		int x = -10, y = 0;
		...
		ChangeMenuState(hwnd, ID_FORMATION_O, MFS_UNCHECKED);
		ChangeMenuState(hwnd, ID_FORMATION_L, MFS_UNCHECKED);
		ChangeMenuState(hwnd, ID_FORMATION_V, MFS_CHECKED);
	}
	break;
